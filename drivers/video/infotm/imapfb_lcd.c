...
#if defined(CONFIG_IMAP_PRODUCTION_0702)
#define IMAPFB_HFP		40//8		/* front porch */
#define IMAPFB_HSW		48//3		/* hsync width */
#define IMAPFB_HBP		40//13		/* back porch */

#define IMAPFB_VFP		13//5		/* front porch */
#define IMAPFB_VSW		3//1		/* vsync width */
#define IMAPFB_VBP		29//7		/* back porch */
...
#elif defined(CONFIG_IMAP_PRODUCTION_7901) || defined(CONFIG_IMAP_PRODUCTION_S01)
...
#define IMAPFB_HFP		15//12		/* front porch */
#define IMAPFB_HSW		104//68		/* hsync width */
#define IMAPFB_HBP		128//81		/* back porch */

#define IMAPFB_VFP		5//10		/* front porch */
#define IMAPFB_VSW		4		/* vsync width */
#define IMAPFB_VBP		21//14		/* back porch */
...
imapfb_fimd_info_t imapfb_fimd = {
	...
	.lcdcon5 = (36<< 24) | IMAP_LCDCON5_INVVCLK_FALLING_EDGE | IMAP_LCDCON5_INVVLINE_INVERTED | IMAP_LCDCON5_INVVFRAME_INVERTED | IMAP_LCDCON5_INVVD_NORMAL
#else
...
void imapfb_backlight_power_supply(UINT32 on_off)
{
        ...
        if(on_off)
        {
                ...
                // Backlight(GPF6)
                msleep(250);
                __raw_writel((((__raw_readl(rGPFCON)) & ~(3<<12)) | (2<<12)), rGPFCON);
...
#if defined(CONFIG_IMAP_PRODUCTION_0702)
	writel((6 << 10) | (2 << 8) | (6 << 2) | (2 << 0), rDIV_CFG4);
#else
...
void imapfb_kernel_logo(void *buf)
{
	...
	memset(pFB, 0, IMAPFB_HRES * IMAPFB_VRES * 2);
	memcpy(pFB + 209 * IMAPFB_HRES * 2, pDB, sizeof(logo_syl));

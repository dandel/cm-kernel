...
#define IMAP_BAT_FULL 645
//#define IMAP_BAT_CRITICAL 511
#define IMAP_BAT_CRITICAL 480
...
{
        unsigned long tmp;
        tmp = (__raw_readl(rGPBDAT)) & 0x2;   //gpb1 ,2 adaptor in, 0 adaptor out
        printk("isacon is %d\n",tmp);
	return tmp;
}*/
   ...
   union power_supply_propval *val)
{
	...
	switch (psp)
	{
		case POWER_SUPPLY_PROP_ONLINE:
                        if (ischargingfull()&&batt > 670)  //|| (ischargingfull() && batt > 620 ))
                        {
...
*/
       ...
       if (ischargingfull()&& batt >670){//charging
		adaptor_disconnect_num =0;
		if (battery_state == BATT_INITIAL){
//				printk("cha zhe dianyuan kaiji chongdian\nbatt is %d\nold_batt is %d\n",batt,old_batt);
			...
			if (batt > 670 && battery_state != BATT_INITIAL){
//	               printk("zhong tu chongdian\nbatt is %d\nold_batt is %d\n",batt,old_batt);
		        old_batt = batt; 
			power_supply_changed(&android_bat);		
                }
		...
		{power_supply_changed(&android_bat);}
        if(old_batt > batt && battery_state == BATT_INITIAL){//initializing
//        	printk("kai ji,wei chongdian\nbatt is %d\nold_batt is %d\n",batt,old_batt);
		old_batt = batt;
                power_supply_changed(&android_bat); 
                battery_state = BATT_ON;
	}
        else if (((old_batt > 670) || (batt > 670)) && battery_state == BATT_ON){
//	        printk("(old_batt > 670) || (batt > 670)\nbatt is %d\nold_batt is %d\n",batt,old_batt);
		old_batt = IMAP_BAT_FULL;
                ...
                if (adaptor_disconnect_num ==2){ 
                        ...
                        if (old_batt >= batt&&batt<660) {
//				printk("chong dian ba diao\nbatt is %d\nold_batt is %d\n",batt,old_batt);
		                old_batt = batt;
		                power_supply_changed(&android_bat);
                        }
                        adaptor_disconnect_num ++;
...
//      printk("(old_batt - batt < 3) && (old_batt - batt >=0)&&(battery_state = BATT_ON)\nbatt is %d\nold_batt is %d\n",batt,old_batt);
	old_batt = batt; 
        power_supply_changed(&android_bat);
        }
        } 

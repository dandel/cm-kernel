...
#endif
	/* free dma resources */
	dma_free_coherent(db->dev, SIZE_OF_DMA_SPA, db->eth_base_txdes_vir, ETH_BASE_TXDES_PHY);
...
static int __devinit gmac_probe(struct platform_device *pdev)
{
	...
	for (i=0; i<4; i++)
	{
		get_random_bytes(&enet_addr[i+2], 1);
	}
#endif
	...
	db->eth_base_txdes_vir = (volatile unsigned int *)dma_alloc_coherent(db->dev, SIZE_OF_DMA_SPA, &ETH_BASE_TXDES_PHY, GFP_KERNEL);

...
static struct class	*encode_class;		/* char device for imapx200 encode */
static struct clk	*imap_venc_clock;	/* hardware clock */
static unsigned int	encode_open_count;	/* record encode open times */
...
/* ioctl */
static int imapx200_encode_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	int ret;
	int out;
...
static int imapx200_encode_probe(struct platform_device *pdev)
{
	...
	encode_param.reg_base_virt_addr = NULL;
...
static int imapx200_encode_remove(struct platform_device *pdev)
{
	...
	/* release resource */
	release_mem_region(encode_param.reg_base_phys_addr, encode_param.reg_reserved_size);
...
{
	struct device *encdev;

	/* encode char device node create */
	encode_param.major = register_chrdev(/* ENCODE_DYNAMIC_MAJOR */ENCODE_DEFAULT_MAJOR, "imapx200-venc", &imapx200_encode_fops);
	...
	encode_param.dev = MKDEV(encode_param.major, /* ENCODE_DYNAMIC_MINOR */ENCODE_DEFAULT_MINOR);

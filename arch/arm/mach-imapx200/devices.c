...
#ifdef CONFIG_ANDROID_PMEM	/* added by sololz */
...
static struct gpio_keys_button imapx200_buttons[] = {
	...
	{
#if defined(CONFIG_IMAP_PRODUCTION_7901) || defined(CONFIG_IMAP_PRODUCTION_S01)
		.gpio		= IRQ_EINT4,	/* INT30 */
#endif
...
/*
 * ABANDONED: DON'T SET CONFIG_ANDROID_PMEM
 * modified by sololz
 * This platform device provided by Google Android system. This 
 * char device driver is designed to be used by media decode and 
 * encode, also display. Provides physical linear memory, all 
 * allocated buffer is supposed to be 4KB align. Pmem is not the
 * only method to alloc physical linear memory, we also use 
 * memalloc to dynamicly do allocation. The reason we use memalloc
 * to cooperate with pmem, because of pmem based on reserved memory,
 * it might be a lot waste of memory if media play is not used.
 * So we suppose that reserved memroy may not be quite so enough,
 * once if pmem is oom, we have to use memalloc to dynamicly do
 * allocation.
 */
...
/* EXPORT_SYMBOL(android_pmem_adsp_device); */	/* modified by sololz */
#endif
